...
| 예측 | 플레이어는 하루에 한 번, `HH:MM:SS` 로 **초까지** 퇴근시간을 예측 (기본값 18:00) |
...
닉네임은 한글·영문·숫자 **10글자 이내**, 프로필은 **한 글자**(이모지 가능)로 각자
`/account` 에서 바꿉니다.
...
| `/account` | 닉네임·프로필 설정 / 비밀번호 변경 / 로그아웃 |
...
| `POST` | `/api/profile` | 로그인 | 닉네임 / 프로필 글자 변경 |
