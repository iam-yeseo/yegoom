...
예전 버전으로 만든 D1 이 있다면 배포 후 한 번만 옮겨 줍니다. 지난 기록은 그대로
옮겨지고(분 → 초, 점수 자동 계산), 이미 최신이면 아무 일도 일어나지 않습니다.

계정도 함께 정리됩니다. `admin` 계정이 없으면 만들고(`admin` / `admin`), 기존 운영자는
플레이어로 내린 뒤 그중 한 명을 출제자로 둡니다. 예전에는 운영자가 곧 출제자였기
때문입니다. 바꾸고 싶으면 `/setup` 에서 출제자를 다시 지정하면 됩니다.

- `/setup` 페이지의 **스키마 업데이트 실행** 버튼 (운영자로 로그인한 상태이거나 `?token=...`)
- 또는 `curl -X POST https://내앱.workers.dev/api/migrate -H "x-setup-token: ..."`
